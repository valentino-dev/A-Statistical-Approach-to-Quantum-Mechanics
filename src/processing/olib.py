...
def plotData(axis, table, label=None, capsize=1, elinewidth=0.5, fmt=",", polyfit=1, color=None, rescale=True, errorbar=True, print_data=True, data_path="data.txt"):
    if rescale:
        X = table.X*table.x_scaling
        Xerr = table.Xerr*table.x_scaling
        Y = table.Y*table.y_scaling
        Yerr = table.Yerr*table.y_scaling
    else:
        X = table.X
        Xerr = table.Xerr
        Y = table.Y
